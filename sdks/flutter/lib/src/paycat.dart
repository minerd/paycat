...
/// PayCat Flutter SDK
class PayCat {
  static PayCat? _instance;
  static PayCat get instance {
    if (_instance == null) {
      throw PayCatNotConfiguredException();
    }
    return _instance!;
  }
  ...
  static Future<PayCat> configure({
    required String apiKey,
    String? appUserID,
    String baseURL = 'https://paycat.ongoru.workers.dev',
  }) async {
    if (_instance != null) {
      return _instance!;
    }
  ...
  Future<void> _initialize() async {
    ...
    _purchaseSubscription = _iap.purchaseStream.listen(
      _onPurchaseUpdated,
      onError: (error) {
        debugPrint('PayCat: Purchase stream error: $error');
      },
    );
    ...
    try {
      ...
      debugPrint('PayCat: Failed to fetch initial subscriber info: $e');
    }
  }
  ...
  Future<List<ProductDetails>> getProducts(Set<String> productIDs) async {
    ...
    if (!available) {
      throw PayCatException('store_unavailable', 'In-app purchases are not available');
    }

    final response = await _iap.queryProductDetails(productIDs);
    if (response.error != null) {
      throw PayCatException('product_query_failed', response.error!.message);
    }
  ...
  Future<SubscriberInfo> purchase(ProductDetails product) async {
    ...
    if (!result) {
      throw PayCatPurchaseCancelledException();
    }
    ...
    Future.delayed(const Duration(seconds: 60), () {
      if (!completer.isCompleted) {
        ...
        completer.completeError(
          PayCatException('purchase_timeout', 'Purchase timed out'),
        );
      }
    });
    ...
    for (final purchase in purchases) {
      if (purchase.status == PurchaseStatus.purchased ||
        ...
        try {
          ...
          debugPrint('PayCat: Failed to sync purchase: $e');
        }
      } else if (purchase.status == PurchaseStatus.error) {
        debugPrint('PayCat: Purchase error: ${purchase.error}');
      }
    }
  }
  ...
  Future<http.Response> _apiRequest(
    ...
    final headers = {
      ...
      'X-API-Key': _apiKey,
      'User-Agent': 'PayCat-Flutter/1.0.0',
    };
    ...
    switch (method) {
      ...
      case 'POST':
        ...
        break;
      default:
        throw PayCatException('invalid_method', 'Invalid HTTP method: $method');
    }

    if (response.statusCode >= 400) {
      try {
        final error = jsonDecode(response.body);
        throw PayCatApiException(
          error['error']['code'] ?? 'unknown',
          error['error']['message'] ?? 'Request failed',
        );
      } catch (e) {
        if (e is PayCatException) rethrow;
        throw PayCatHttpException(response.statusCode);
      }
    }
