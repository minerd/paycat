 ...
 * MRRCat API Client
 */
internal class MRRCatApiClient(
    ...
    suspend fun getSubscriber(appUserID: String): SubscriberInfo = withContext(Dispatchers.IO) {
        val request = Request.Builder()
            ...
            .addHeader("User-Agent", "MRRCat-Android/1.0.0")
            .get()
            .build()

        val response = client.newCall(request).execute()
        val body = response.body?.string() ?: throw MRRCatException.NetworkError()
    ...
    private fun handleError(code: Int, body: String): Nothing {
        try {
            ...
            throw MRRCatException.ApiError(error.error.code, error.error.message)
        } catch (e: Exception) {
            if (e is MRRCatException) throw e
            throw MRRCatException.HttpError(code)
        }
    }
