...
{
    /// <summary>
    /// PayCat SDK for Unity
    /// Unified subscription management across iOS, Android, and Web
    /// </summary>
    public class PayCatSDK : MonoBehaviour
    {
        private static PayCatSDK _instance;
        public static PayCatSDK Instance => _instance;
        ...
        /// <summary>
        /// Initialize PayCat SDK with configuration
        /// </summary>
        public static void Configure(string apiKey, string serverUrl = null)
        {
            if (_instance == null)
            {
                var go = new GameObject("PayCat");
                _instance = go.AddComponent<PayCatSDK>();
            }
        ...
        private async Task<T> MakeRequest<T>(string method, string url, object body = null)
        {
            using (var request = new UnityWebRequest(url, method))
            {
                ...
                if (request.result != UnityWebRequest.Result.Success)
                {
                    ...
                    throw new PayCatException(errorMessage, (int)request.responseCode);
                }
        ...
        private static void Log(string message)
        {
            ...
            {
                Debug.Log($"[PayCat] {message}");
            }
        }

        private static void LogError(string message)
        {
            Debug.LogError($"[PayCat] {message}");
        }
    ...
    public class PayCatException : Exception
    {
        public int StatusCode { get; }

        public PayCatException(string message, int statusCode = 0) : base(message)
        {
