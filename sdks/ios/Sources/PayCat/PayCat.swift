...
public final class PayCat: @unchecked Sendable {

    // MARK: - Singleton

    /// Shared instance of PayCat
    public static let shared = PayCat()

    // MARK: - Properties

    private var apiKey: String?
    private var appUserID: String?
    private var baseURL: String = "https://paycat.ongoru.workers.dev"
    ...
    public weak var delegate: PayCatDelegate?
    ...
    private func refreshSubscriberInfo() async throws -> SubscriberInfo {
        ...
        DispatchQueue.main.async {
            self.delegate?.paycat(self, didReceiveUpdated: subscriberInfo)
        }
    ...
    public func purchase(productID: String) async throws -> SubscriberInfo {
        ...
        guard let product = products.first else {
            throw PayCatError.productNotFound(productID)
        }
    ...
    public func purchase(product: Product) async throws -> SubscriberInfo {
        ...
        case .success(let verification):
            ...
            case .unverified(_, let error):
                throw PayCatError.verificationFailed(error.localizedDescription)
            }

        case .userCancelled:
            throw PayCatError.purchaseCancelled

        case .pending:
            throw PayCatError.purchasePending

        @unknown default:
            throw PayCatError.unknown
        }
    }
    ...
    /// Sync a transaction with PayCat backend
    @available(iOS 15.0, macOS 12.0, tvOS 15.0, watchOS 8.0, *)
            ...
            for await result in Transaction.updates {
                ...
                if case .verified(let transaction) = result {
                    do {
                        ...
                        await MainActor.run {
                            self.delegate?.paycat(self, didReceiveUpdated: info)
                        }
                    } catch {
                        await MainActor.run {
                            self.delegate?.paycat(self, didReceiveError: error)
                        }
                    }
                }
            }
    ...
    private func generateAnonymousID() -> String {
        if let stored = UserDefaults.standard.string(forKey: "paycat_anonymous_id") {
            return stored
        }
        let id = "$anon_\(UUID().uuidString.lowercased())"
        UserDefaults.standard.set(id, forKey: "paycat_anonymous_id")
        return id
    }
    ...
    private func apiRequest<T: Decodable>(
        ...
        request.setValue("PayCat-iOS/1.0.0", forHTTPHeaderField: "User-Agent")
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            throw PayCatError.networkError
        }

        guard 200...299 ~= httpResponse.statusCode else {
            if let error = try? JSONDecoder().decode(APIError.self, from: data) {
                throw PayCatError.apiError(error.error.code, error.error.message)
            }
            throw PayCatError.httpError(httpResponse.statusCode)
        }
...
@available(iOS 14.0, macOS 11.0, tvOS 14.0, watchOS 7.0, *)
public protocol PayCatDelegate: AnyObject {
    func paycat(_ paycat: PayCat, didReceiveUpdated subscriberInfo: SubscriberInfo)
    func paycat(_ paycat: PayCat, didReceiveError error: Error)
}

// MARK: - Default Delegate Implementation

@available(iOS 14.0, macOS 11.0, tvOS 14.0, watchOS 7.0, *)
public extension PayCatDelegate {
    func paycat(_ paycat: PayCat, didReceiveError error: Error) {}
}
